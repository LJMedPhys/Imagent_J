...
2. **Initialise minimally — one turn, parallel calls:**
   ...
   multi-channel), `input_files`, and the compact `vlm_assessment`. That single record is enough — do NOT spread
...
6. **Visually judge, show, and report.** After the final image output exists, call
   `vlm_judge` once before declaring completion. For segmentation, pass the exact
   `[original_path, mask_path]` pair and `create_mask_overlay=True`; for another
   transformation pass a labelled before/after pair. Persist the handoff with
