...
2. **Initialise minimally — one turn, parallel calls:**
   ...
   `confocal fluorescence`, `EM`, `microCT`, …).
   
   **If the file and the user's words do not settle it, ASK THE USER** — one short
   question, they always know, and it costs far less than segmenting for the wrong
   contrast mechanism. Never write `"unknown"`: a placeholder is read as no answer.
