...
>
> Use the big buttons in the **ImagentJ — Annotation Helper** panel on the right:
> - **➕ ADD objects** — click the middle of a missed object, press **S**, then press **C**.
> - **✏ DRAW round objects** — click round an object, double-click to close (several is fine),
...
> receipt — `SUMMARY: STILL RUNNING (detached)` — within seconds, and the training output
> arrives later as a separate `[DETACHED RUN FINISHED]` message. That receipt is NOT a
> failure and NOT an empty result: do not re-run the stage, do not poll for it, and do not
> report a score you have not seen. Tell the user training is running and roughly how long
> it takes, then answer whatever they ask next — the point of detaching is that they can
> talk to you during the 10-20 minutes instead of waiting on a blocked turn.
...
21. **You cannot split a tight cluster with the one-click workflow — do not plan a run
    ...
    **Do not try to feed SAM a new KIND of prompt here.** Two attempts failed the same way: a
    ...
    brush — could be interrupted part-way, and the in-progress shape is then discarded by
    napari's `_finish_drawing()`. Suspending those callbacks while the tool was armed did not
