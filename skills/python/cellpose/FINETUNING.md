...
> `SUMMARY: STILL RUNNING (detached)` within seconds and the training output arrives later as
> a separate `[DETACHED RUN FINISHED]` message. That receipt is not a failure and not an empty
