...
// Reference: official SNT script templates Sholl_Extract_Profile_From_Image_Demo.py,
// Sholl_Bitmap_vs_Tracing_Comparison.groovy
...
#@ File(label = "Output profile CSV", style = "save", value = "/data/snt_output/image_sholl_profile.csv") profileCsvFile
#@ File(label = "Output Sholl stats CSV", style = "save", value = "/data/snt_output/image_sholl_stats.csv") statsCsvFile
...
/*
 * SNT - Image-based Sholl analysis
 *
 * PURPOSE:
 *   1. Load a segmented 2D or 3D image
 *   2. Parse the image into a Sholl profile using ImageParser2D or ImageParser3D
 *   3. Save the sampled profile (radius, count) as CSV
 *   4. Compute summary statistics, polynomial fit, and normalized Sholl decay
 *   5. Save all summary metrics as CSV
 *
 * REQUIRED INPUTS:
 *   inputImageFile - segmented image file, or leave unset and enable the sample image fallback
 *   profileCsvFile - new CSV path for sampled radius/count pairs
 *   statsCsvFile   - new CSV path for summary metrics
 *
 * IMPORTANT:
 *   - This workflow expects a segmented or thresholdable image, not an SWC reconstruction.
 *   - If the image is not already binary or thresholded, provide manual thresholds or
 *     use an automatic threshold method.
 *   - Centers are specified in pixel coordinates. Negative center coordinates fall back
 *     to the geometric center of the image.
 *   - For 3D images, the parser can ignore isolated single voxels before counting intersections.
 */
...
ImagePlus loadImage() {
    ...
    if (useSampleImageIfInputMissing) {
        ...
        if (imp == null) {
            throw new IllegalStateException("SNT sample image could not be obtained")
        }
        return imp
    }
...
void configureThreshold(ImagePlus imp, def parser) {
    ...
    if ("none".equals(autoThresholdMethod)) {
        throw new IllegalArgumentException("Image is not binary/thresholded. Provide manual thresholds or enable auto-thresholding.")
    }
...
println("SNT image-based Sholl analysis")
...
println("SNT image-based Sholl analysis complete")
