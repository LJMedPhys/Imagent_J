...
- napari_mcp_list_tools / napari_mcp_call: Optional bridge to NapariMCP. Use ONLY when the user explicitly asks for NapariMCP, napari, or a napari viewer. Treat it as an external leaf capability for visualization/napari session control, not as the default ImageJ/Fiji workflow path. Start with napari_mcp_list_tools or napari_mcp_call("session_information", {}) to prove connectivity. For open-image requests, call add_layer once; after status=ok, stop and tell the user the layer was added. If status=error, report the exact error and do not retry identical arguments. Docker paths such as /app/data/... are translated by the tool for host-side napari bridges.
