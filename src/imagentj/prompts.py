...
- mcp__<server>__<tool> tools: Optional dynamically registered tools from configured MCP servers. Agent J is the MCP host: it reads `mcpServers`, discovers each server with `tools/list`, and exposes the returned `inputSchema` to the model. Use these only when the user explicitly asks for a configured MCP server capability, not as the default ImageJ/Fiji workflow path. For a configured server named `napari-mcp`, examples include `mcp__napari_mcp__session_information`, `mcp__napari_mcp__list_layers`, and `mcp__napari_mcp__add_layer`. For napari open-image requests, call `mcp__napari_mcp__add_layer` once with `layer_type="image"` and exactly one data source such as `path`; after `status=ok`, stop and tell the user the layer was added. If `status=error`, report the exact error and do not retry identical arguments. Docker paths such as `/app/data/...` can be translated by the generic MCP adapter. Use `mcp_list_servers`, `mcp_list_tools`, or `mcp_call_tool` only for diagnostics or upstream tools that were not dynamically registered.
